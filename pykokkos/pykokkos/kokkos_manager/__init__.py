...
for space in ExecutionSpace:
    ...
    # Map debug spaces to their underlying implementation for Kokkos
    from pykokkos.interface.execution_space import get_underlying_execution_space
    underlying_space = get_underlying_execution_space(space)

    # Check if the underlying execution space is available before creating instances
    if not kokkos.get_device_available(underlying_space):
        continue

    CONSTANTS["AVAILABLE_EXECUTION_SPACES"][space] = ExecutionSpaceInstance(space)

    # Map debug spaces to their underlying GPU framework and create corresponding instances
    debug_to_underlying = {
        ExecutionSpace.DebugCuda: (ExecutionSpace.Cuda, "CUDA"),
        ExecutionSpace.DebugHIP: (ExecutionSpace.HIP, "HIP"),
        ExecutionSpace.DebugOpenMP: (ExecutionSpace.OpenMP, "OpenMP")
    }

    if space in debug_to_underlying:
        underlying, key = debug_to_underlying[space]
        CONSTANTS["AVAILABLE_EXECUTION_SPACES"][key] = ExecutionSpaceInstance(underlying)
        if underlying in {ExecutionSpace.Cuda, ExecutionSpace.HIP}:
            CONSTANTS["GPU_BACKEND"] = underlying
    elif space in {ExecutionSpace.Cuda, ExecutionSpace.HIP}:
        # Set GPU_BACKEND for actual GPU spaces (not debug variants)
