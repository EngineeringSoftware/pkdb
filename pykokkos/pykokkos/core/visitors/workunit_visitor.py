    ...
    def visit_AnnAssign(self, node: ast.AnnAssign) -> cppast.Stmt:
        if isinstance(node.value, ast.Call):
            ...
            if function_name in ("parallel_reduce", "parallel_scan"):
                ...
                marker_lineno: int = getattr(node, "end_lineno", None) or node.lineno
                line_marker = cppast.LineMarkerStmt(marker_lineno, self.current_file_path)

                return cppast.CompoundStmt([declstmt, callstmt, line_marker])
