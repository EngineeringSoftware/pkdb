...
class PyKokkosVisitor(ast.NodeVisitor):
    def __init__(
        ...
        module_file = self.env.get("__file__")
        self.current_file_path: str = os.path.abspath(module_file) if module_file else ""
    ...
    def visit_body_with_markers(self, body: List[ast.stmt]) -> cppast.CompoundStmt:
        """Helper method to visit a list of statements and add line markers for each line."""
        body_list = []
        for b in body:
            visited = self.visit(b)
            # Nested work-units (and nonlocal) emit no C++ stmt; skip placeholders and
            # avoid pairing a line marker with an empty statement.
            if visited == "":
                continue
            body_list.append(visited)
            if not hasattr(b, "lineno"):
                continue
            # visit() may already append a LineMarkerStmt (e.g. Kokkos parallel_reduce).
            if (
                isinstance(visited, cppast.CompoundStmt)
                and visited.statements
                and isinstance(visited.statements[-1], cppast.LineMarkerStmt)
            ):
                continue
            body_list.append(cppast.LineMarkerStmt(b.lineno, self.current_file_path))
        return cppast.CompoundStmt(body_list)
    ...
    def visit_For(self, node: ast.For) -> cppast.ForStmt:
        ...
        body = self.visit_body_with_markers(node.body)
    ...
    def visit_If(self, node: ast.If) -> cppast.IfStmt:
        ...
        then_body = self.visit_body_with_markers(node.body)
        else_body = (
            self.visit_body_with_markers(node.orelse)
            if node.orelse
            else None
        )
    ...
    def error(self, node, message):
        visitors_util.error(self.src, self.debug, node, message, self.current_file_path)
