...
from pykokkos.interface import (
    ...
    get_underlying_execution_space,
    is_debug_execution_space,
...
class CppSetup:
    ...
    def compile_raw_source(
        ...
        self.write_raw_source(output_dir, source, filename, space)
    ...
    def write_source(
        ...
        self.write_raw_source(output_dir.parent, functor, functor_filename, space)
        self.write_raw_source(output_dir.parent, functor_cast, functor_cast_filename, space)
        self.write_raw_source(output_dir, bindings, bindings_filename, space)
    ...
    def write_raw_source(
        ...
        :param space: when set, debug execution spaces trigger formatting
        """
        ...
        debug_format = space is not None and is_debug_execution_space(space)
        if self.format or debug_format:
            try:
                cmd = ["clang-format", "-i", str(file_path)]
                if debug_format:
                    cmd[1:1] = ["-style={ColumnLimit: 800}"]
                subprocess.run(cmd)
    ...
    def generate_cmake(
        ...
        if space in {ExecutionSpace.Cuda, ExecutionSpace.DebugCuda}:
            view_space = "Kokkos::CudaUVMSpace" if enable_uvm else "Kokkos::CudaSpace"
            view_layout = "Kokkos::LayoutLeft"
        elif space in {ExecutionSpace.HIP, ExecutionSpace.DebugHIP}:
            view_space = (
        ...
        # Kokkos has no DebugCuda/DebugHIP types; use underlying execution space for C++ macros
        kokkos_space: str = get_underlying_execution_space(space)
        space_value: str = "Experimental::HIP" if kokkos_space == "HIP" else kokkos_space
        ...
        try:
            ...
            print(f"Can not get pybind11 cmake dir: {ex}")
            sys.exit(1)
        ...
        # Match CMake project() to Kokkos execution space (not global compiler default).
        if compiler == "nvcc" and kokkos_space == "Cuda":
            cmake_args.append("-DENABLE_CUDA=ON")
        elif compiler == "hipcc" and kokkos_space == "HIP":
            cmake_args.append("-DENABLE_HIP=ON")

        if is_debug_execution_space(space):
            cmake_args.append("-DPK_DEBUG_BUILD=ON")
