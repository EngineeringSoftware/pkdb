...
from .stmt import (
    ...
    IfStmt, LineMarkerStmt, ReturnStmt, WhileStmt
)
...
class Serializer:
    ...
    def serialize_CompoundStmt(self, node: CompoundStmt) -> str:
        ...
        for i, stmt in enumerate(node.statements):
            ...
            # check if there is a marker after stmt
            has_marker = i + 1 < len(node.statements) and isinstance(node.statements[i + 1], LineMarkerStmt)
            if has_marker:
                marker = node.statements[i + 1]
                stmt_str = self._serialize_stmt_with_marker(stmt, marker)
                skip_next = True
            else:
                stmt_str = self.serialize(stmt)
            statements.append(stmt_str)

        return "".join(statements)

    def _serialize_stmt_with_marker(self, stmt: Node, marker: LineMarkerStmt) -> str:
        """Serialize a statement with its line marker inline."""
        stmt_str = self.serialize(stmt)
        marker_comment = self.serialize(marker).strip()  # Reuse existing serializer, remove newline

        # compound stmt, insert marker after opening brace
        if isinstance(stmt, (IfStmt, ForStmt, WhileStmt)):
            brace_pos = stmt_str.find("{")
            if brace_pos != -1:
                return stmt_str[: brace_pos + 1] + f" {marker_comment}\n" + stmt_str[brace_pos + 1 :]
            return stmt_str

        # return `<line>; // marker`
        return stmt_str.rstrip(";\n") + f"; {marker_comment}\n"
    ...
    def serialize_IfStmt(self, node: IfStmt) -> str:
        ...
        stmt: str = f"if ({condition}){{ {then_body} }}"

        if not node.else_body:
            return stmt

        # Check if else body starts with an elif (another IfStmt)
        if isinstance(node.else_body, CompoundStmt) and node.else_body.statements:
            first_stmt = node.else_body.statements[0]

            if isinstance(first_stmt, IfStmt):
                # elif case
                else_body = self.serialize(node.else_body)
                return stmt + f"else {else_body}"

            if isinstance(first_stmt, LineMarkerStmt):
                # else case
                marker_comment = self.serialize(first_stmt).strip()
                rest_body = self.serialize(CompoundStmt(node.else_body.statements[1:]))
                return stmt + f"else {{{marker_comment}\n{rest_body} }}"

        # Standard else clause
        else_body = self.serialize(node.else_body)
        return stmt + f"else {{ {else_body} }}"
    ...
    def serialize_LineMarkerStmt(self, node: LineMarkerStmt) -> str:
        # Generate a unique marker that clang-format will preserve
        lineno = node.python_lineno
        file_path = node.file_path or ""
        hash_input = f"{file_path}:{lineno}".encode("utf-8")
        hash_obj = hashlib.sha256(hash_input)
        simple_hash = hash_obj.hexdigest()[:8]
        # Encode both the Python source file path and line so downstream tools
        # can reconstruct mappings without relying on pkdb-side analyzers.
        # Format: // PY_LINE_MARKER_<file_path>_<python_lineno>_<hash>
        return f"// PY_LINE_MARKER_{file_path}_{lineno}_{simple_hash}\n"
