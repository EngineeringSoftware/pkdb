...
# iinfo/finfo use substring tests and do not cover bare "int"; finfo treats bare
# "float" as 32-bit, while PyKokkos scratch / Kokkos layouts use 64-bit double.
_ITEMSIZE_OVERRIDE: dict[str, int] = {
    "int": 4,
    "float": 8,
}


def dtype_itemsize_bytes(type_name: str) -> int:
    """
    sizeof(scalar) in bytes for dtypes named in Python (e.g. ``@workunit scratch=[(dtype, ...)]``).

    Reuses :func:`iinfo` and :func:`finfo` where those match; otherwise applies
    small overrides and defaults to 8 bytes.
    """

    if type_name in _ITEMSIZE_OVERRIDE:
        return _ITEMSIZE_OVERRIDE[type_name]
    ii = iinfo(type_name)
    if ii is not None:
        return ii.bits // 8
    fi = finfo(type_name)
