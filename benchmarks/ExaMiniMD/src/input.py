...
class Input:
    def __init__(self, p: System):
        ...
        self.atoms_override: int = 0  # If > 0, override lattice size (cubic) to get this many atoms
    ...
    def read_command_line_args(self) -> None:
        ...
        parser.add_argument(
            ...
            metavar="N",
            default=0,
            help="Override number of atoms with a cubic box (from input file region). E.g. 4000, 32000, 108000, 256000, 500000.",
        )
        ...
        if args.atoms > 0:
            self.atoms_override = args.atoms
    ...
    def read_lammps_file(self, filename: str) -> None:
        ...
        # Override lattice size if --atoms N was passed (cubic box)
        if self.atoms_override > 0:
            if self.lattice_style == LatticeType.LATTICE_FCC.value:
                n = max(1, int(round((self.atoms_override / 4) ** (1.0 / 3.0))))
            else:
                n = max(1, int(round(self.atoms_override ** (1.0 / 3.0))))
            self.lattice_nx = self.lattice_ny = self.lattice_nz = n
            if self.system.do_print:
                print(f"# --atoms {self.atoms_override} -> region block 0 {n} 0 {n} 0 {n}")
