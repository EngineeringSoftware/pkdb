...
class ExaMiniMD:
    def __init__(self):
        # Match run_eval/run_examinimd: prefer PK_EXEC_SPACE, fall back to PK_EXA_SPACE
        space: Optional[str] = os.environ.get("PK_EXEC_SPACE") or os.environ.get("PK_EXA_SPACE")
        if space is not None:
            pk.set_default_space(pk.ExecutionSpace(space))
            if space in {"Cuda", "DebugCuda", "HIP", "DebugHIP"}:
                pk.enable_uvm()
        print(f"run under {space}")
        self.system = System()
        self.system.init()
    ...
    def run(self, nsteps: int) -> None:
        ...
        profile_path = os.environ.get("PK_KERNEL_PROFILE_PATH", "kernel_profile.json")
        profiler.write_json(profile_path)
        if self.system.do_print:
            print(f"Kernel profile written to {profile_path}")
