...
class ForceLJNeigh(Force):
    ...
    def t_fparams(x: int = 0, y: int = 0, data_type: pk.DataTypeClass = pk.double):
        if x > 0 and y > 0:
            # LayoutRight matches C-order indexing: coeff[type_i][type_j]
            return pk.View([x, y], data_type, layout=pk.Layout.LayoutRight)
        import numpy as np
    ...
    def compute(self, system: System, binning: Binning, neighbor: Neighbor) -> None:
        ...
        with profiler.record("ForceLJNeigh::compute"):
            if self.half_neigh:
                pk.parallel_for(
    ...
    def compute_energy(
        ...
        with profiler.record("ForceLJNeigh::compute_energy"):
            if self.half_neigh:
