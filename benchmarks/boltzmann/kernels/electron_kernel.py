...
def xsection_kernel(
    ...
    d_coll_indicator_g0_ar: pk.View2D[pk.int64],
    # d_coll_indicator_g0_ar: pk.View1D[pk.int64],
    # d_coll_indicator_g1_ar: pk.View1D[pk.int64],
    # d_coll_indicator_g2_ar: pk.View1D[pk.int64],
    # d_coll_indicator_g3_ar: pk.View1D[pk.int64],
    nn: float,
    dt: float,
    stride: int,
    zero_g2: bool,
    ...
    for i in range(tid, Nc, stride):
        ...
        i10: float = 10.63765
        log_sig_g0: float = (
            (e_el < i1) * (((a1 * log_e_el + b1) * log_e_el + c1))
        ...
        log_sig_g2: float = (
            (e_ion_full < e_el and e_el < i1) * (((a1 * log_e_el + b1) * log_e_el + c1))
            + (i1 <= e_el and e_el < i2) * (((a2 * log_e_el + b2) * log_e_el + c2))
            + (i2 <= e_el and e_el < i3) * (((a3 * log_e_el + b3) * log_e_el + c3))
            + (i3 <= e_el and e_el < i4) * (((a4 * log_e_el + b4) * log_e_el + c4))
            + (i4 <= e_el and e_el < i5) * (((a5 * log_e_el + b5) * log_e_el + c5))
        ...
        d_coll_indicator_g0_ar[0][i] = 0 + 1 * (d_R_vec[i][0] < sig_range_g0)
        d_coll_indicator_g0_ar[1][i] = 0 + 1 * (
            sig_range_g0 < d_R_vec[i][0] and d_R_vec[i][0] < sig_range_g1
        )
        d_coll_indicator_g0_ar[2][i] = 0 + 1 * (
            sig_range_g1 < d_R_vec[i][0] and d_R_vec[i][0] < sig_range_g2
        )
        d_coll_indicator_g0_ar[3][i] = 0 + 1 * (
            sig_range_g2 < d_R_vec[i][0] and d_R_vec[i][0] < sig_range_g3
        )
...
def collision_kernel(
    ...
    for i in range(tid, Nc, stride):
        ...
        ## G0: ELASTIC
        if d_coll_indicator_g0_ar[0][i] == 1:
            ...
            sign_sintheta_g0: float = (d_R_vec[i][4] > 0.5) * 1 + (
                d_R_vec[i][4] < 0.5
            ) * (-1)
            # TERM 1
            ...
            # TERM 2
            fac: float = (
                math.sin(chi) * math.sin(phi) / (math.sin(theta) * sign_sintheta_g0)
            )
            v_scat_x += 0
        ...
        ## G1: EXCITATION
        elif d_coll_indicator_g0_ar[1][i] == 1:
            ## Original electron deflection direction (vscat)
            # cos_chi = 1 - (2*d_R_vec[i][1]) / (1 + 8*e_el*(1 - d_R_vec[i][1]))
            cos_chi: float = 2 * d_R_vec[i][1] - 1
            chi: float = math.acos(cos_chi)
        ...
        ## G2 & G3: IONIZATION & STEPWISE
        elif d_coll_indicator_g0_ar[2][i] == 1 or d_coll_indicator_g0_ar[3][i] == 1:
            ## Original electron deflection direction (vscat)
            # cos_chi = 1 - (2*d_R_vec[i][1]) / (1 + 8*e_el*(1 - d_R_vec[i][1]))
            cos_chi: float = 2 * d_R_vec[i][1] - 1
            chi: float = math.acos(cos_chi)
            ...
            # Energy splitting
            e_ej: float = d_coll_indicator_g0_ar[2][i] * abs(
                0.5 * (e_el - e_ion_full)
            ) + d_coll_indicator_g0_ar[3][i] * abs(0.5 * (e_el - e_ion_step))
            e_scat: float = e_ej
            ...
            d_tosum_ar[i][ai] = 1.0
            if d_coll_indicator_g0_ar[2][i]:
                d_collct_ar[i][2] = 1.0
                d_g2_ar[i] = 1.0
            d_tosum_ar[i][ae] = d_coll_indicator_g0_ar[3][i] * (-1.0)
            if d_coll_indicator_g0_ar[3][i]:
                d_collct_ar[i][3] = 1.0
                d_g3_ar[i] = 1.0
            ...
            sign_sintheta_g2: float = (d_R_vec[i][6] > 0.5) * (1.0) + (
                d_R_vec[i][6] < 0.5
            ) * (-1.0)
            # TERM 1
...
def advection_kernel(
    ...
    for i in range(tid, Nc, stride):
        ...
        d_data_ar[i][
            xx
        ] = d_xx  # don't kill for cuda reduction purposes with empty spot (long story)
...
def electron_kernel(
    ...
    d_g2_ar,
    d_g3_ar,
    d_coll_indicator_g0_ar,
    # d_coll_indicator_g1_ar,
    # d_coll_indicator_g2_ar,
    # d_coll_indicator_g3_ar,
    zero_g2: bool,
):
    ...
    pk.parallel_for(
        ...
        d_coll_indicator_g0_ar=d_coll_indicator_g0_ar,
        # d_coll_indicator_g1_ar=d_coll_indicator_g1_ar,
        # d_coll_indicator_g2_ar=d_coll_indicator_g2_ar,
        # d_coll_indicator_g3_ar=d_coll_indicator_g3_ar,
        nn=nn,
        dt=dt,
        stride=num_threads,
        zero_g2=zero_g2,
