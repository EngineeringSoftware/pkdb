...
MARKER_PREFIXES = ("debug_comparison_", "debug-comparison-")
...
def _try_debug_compare(
    ...
    for entry in sizes:
        ...
        for k in debug_keys:
            b = by_backend.get(k)
            v = b.get("normal_avg") if b else None
            if v is None:
                row_ok = False
                break
            row_vals[k] = float(v)
...
def build_figure(prefix: str, plat: Platform) -> str:
    ...
    cap = (
        ...
        f"(old debugger vs \\ourTool{{}}) on {plat.label}."
    )
...
def build_chart_tex(prefix: str, platforms: List[Platform]) -> str:
    if not platforms:
        return (
            "% Auto-generated by paper/reports/scripts/eval-debug-chart.py\n"
